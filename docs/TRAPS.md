...
The long form of CLAUDE.md's two rule sections, moved here verbatim on
2026-09-24 when CLAUDE.md passed Claude Code's instruction-size limit.
CLAUDE.md keeps each entry's headline and a pointer to its heading here;
the measurement, the mechanism and the HISTORY.md citation behind a rule
are here. A comment or doc that cites a rule as "CLAUDE.md" means this
file. Add a new rule here, and its headline to CLAUDE.md.
...
### CLI parity and tool registration
...
### Word indices echo their neighbours
...
### Footage descriptions and pinned cues
...
### Orphaned cues, tails, holds and the music bed
